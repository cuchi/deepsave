...
- **AI proposals** live in `backend/src/services/ai_tags.rs` + `routes/ai_tags.rs` (batches carry `kind`: 'tags' | 'categorize' — migration 0019; `ai_tag_suggestions.suggested_category` holds category proposals, "nova: X" creates the category). Suggestions are reviewed inline in Lista (amber chips, banner with Aplicar/Ignorar todas, "Só itens com sugestão" filter). Tag descriptions (F0, `tags` registry table + `tags::tag_descriptions`) are injected into every prompt.
