...
async fn mount_tag_mock(suggestions: Value) -> MockServer {
    let server = MockServer::start().await;
    let content = json!({ "suggestions": suggestions }).to_string();
...
async fn enqueue_creates_batch_and_suggestions(pool: PgPool) {
    ...
    let batch = ai_tags::enqueue_batch(&pool, vec![a, b]).await.unwrap();
...
async fn enqueue_validation_errors(pool: PgPool) {
    ...
    match ai_tags::enqueue_batch(&pool, vec![]).await {
...
async fn process_batch_fills_suggestions_from_ai(pool: PgPool) {
    ...
    let server = mount_tag_mock(json!([
    ...
    let batch = ai_tags::enqueue_batch(&pool, vec![a, b, c]).await.unwrap();
...
async fn process_batch_gracefully_handles_bad_ai_response(pool: PgPool) {
    ...
    let server = mount_tag_mock(json!([{ "index": "0", "tags": "padaria" }])).await;
    let ai = common::ai_client(pool.clone(), server.uri());

    let batch = ai_tags::enqueue_batch(&pool, vec![a]).await.unwrap();
    ai_tags::process_batch(&pool, &ai, batch.id).await.unwrap();
...
async fn apply_all_and_dismiss_all_scope_to_batch(pool: PgPool) {
    ...
    let batch1 = ai_tags::enqueue_batch(&pool, vec![a, b]).await.unwrap();
    let batch2 = ai_tags::enqueue_batch(&pool, vec![c]).await.unwrap();
